...
def get_short_video_image(AccessKeyId, AccessKeySecret, endpoint, region):
    search_body = {
        "query": {
            "bool": {
                    "filter": [
                            {"range": {"release_time": {"gte": int((datetime.datetime.now() + datetime.timedelta(days=-60)).timestamp()*1e3)}}},
                            {"range": {"play_count": {"gte": 10000}}}
...
if __name__ == '__main__':
    AccessKeyId = "LTAI4Fv7hAqiQmaHbh1JZvFM"
    AccessKeySecret = "FESoligbqzZ1LCHt5bdb4vfJgTjoV9"
