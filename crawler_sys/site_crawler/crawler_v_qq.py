...
class Crawler_v_qq():
    ...
    def video_page(self, url, channel=None):
        ...
        if get_page is None:
            ...
            if soup_find != None:
                ...
                return None
            try:
                releaser_id = get_releaser_id(platform=self.platform,releaserUrl=releaserUrl)
            except:
                releaser_id = ""
            ...
            if play_count != None:
                ...
                if releaserUrl is not None:
                    ...
                    video_dict['releaser_id_str'] = "腾讯视频_%s"%(releaser_id)
            ...
            while pagenum <= releaser_page_num_max:
                ...
                if proxies_num:
                    get_page = retry_get_url(releaser_page_url,
                                             timeout=self.timeout,proxies=proxies_num)
                else:
                    get_page = retry_get_url(releaser_page_url,
    ...
    #        self.loggerinfo.info('releaser: %s, releaserUrl: %s finished' % (releaser, releaserUrl))

    def get_releaser_follower_num(self, releaserUrl):
        proxies = get_proxy(1)
        if proxies:
            get_page = requests.get(releaserUrl,proxies=proxies)
        else:
    ...
    async def releaser_page_async(self):
        pass
...
if __name__ == '__main__':
    test = Crawler_v_qq()
    url = 'http://v.qq.com/vplus/63395e4ccbdd11c3923e3b7fce6a6c54#uin=63395e4ccbdd11c3923e3b7fce6a6c54'
    # releaserUrl = 'http://v.qq.com/vplus/ea1b1b8626787db2d198c58fb86eb4dc/videos'
    # nnn = test.video_page(url)
    # kw = '任正非 BBC'
    # #sr = test.search_page(kw, search_pages_max=2)
    test.releaser_page(url, output_to_es_raw=True,
                        es_index='crawler-data-raw',
                          doc_type='doc',
                         releaser_page_num_max=400,proxies_num=1)
    # releaserUrl=url,)
    # test.get_releaser_follower_num("http://v.qq.com/vplus/63395e4ccbdd11c3923e3b7fce6a6c54#uin=63395e4ccbdd11c3923e3b7fce6a6c54")
