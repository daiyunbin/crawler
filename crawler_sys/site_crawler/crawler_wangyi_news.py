...
class Crawler_wangyi_news():
    ...
    def releaser_page(self, releaserUrl,
        ...
        while count <= releaser_page_num_max and count <= 1000 and pcursor != "no_more":
            ...
            try:
                if proxies:
                    get_page = requests.get(releaserUrl, headers=self.headers, timeout=5,proxies=proxies)
                else:
                    get_page = requests.get(releaserUrl, headers=self.headers,timeout=5)
                # print(data_list)
                # print(releaserUrl)
                page_dic = get_page.json()
...
if __name__ == '__main__':
    ...
    # re = test.get_releaser_follower_num(user_lis[0])
    #print(re)
    # sr_tud = test.search_page(keyword='任正非 BBC', search_pages_max=2)
    test.video_page(url,output_to_es_raw=True,es_index='crawler-data-raw',doc_type='doc')
    # test.releaser_page("https://c.m.163.com/news/sub/T1514817332429.html",output_to_es_raw=True,es_index='crawler-data-raw',doc_type='doc',releaser_page_num_max=500)